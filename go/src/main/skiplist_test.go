...
func init() {
	...
	num := generateRandomNumber(0, 50000000, 50000000)
	benchList = New()


	for i := 0; i <= 50000000; i++ {

		benchList.Set(float64(num[i]), [1]byte{})
	}
...
func TestChangeLevel(t *testing.T) {
	...
	if list.maxLevel != DEFAULT_MAX_LEVEL {
...
func TestMaxLevel(t *testing.T) {
	list := NewWithMaxLevel(DEFAULT_MAX_LEVEL + 1)
	list.Set(0, struct{}{})
}
...
func TestChangeProbability(t *testing.T) {
	...
	if list.probability != DEFAULT_MAX_LEVEL {
